import './globals.css';
import { Montserrat, Roboto_Mono, Poppins } from 'next/font/google';
import { LayoutHeader } from '@/components/LayoutHeader';
import { BurgerMenuProvider } from '@/context/BurgerMenuContext';

const montserrat = Montserrat({
  subsets: ['latin'],
  variable: '--font-montserrat',
  display: 'swap',
});

const robotoMono = Roboto_Mono({
  subsets: ['latin'],
  weight: ['400', '700'],
  style: ['normal'],
  variable: '--font-roboto-mono',
});

const dmSans = Poppins({
  subsets: ['latin'],
  variable: '--font-dm-sans',
  weight: ['400', '500', '700', '800', '900'],
});

export const metadata = {
  title: 'Create Next App',
  description: 'Generated by create next app',
};

export default function RootLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <html
      lang='pt-br'
      className={`${montserrat.variable} ${robotoMono.variable} ${dmSans.variable} dark`}
    >
      <body className='prose max-w-none bg-light dark:prose-invert prose-h1:text-3xl prose-p:font-light prose-a:no-underline prose-ul:p-0 prose-li:list-none prose-li:p-0 dark:bg-dark sm:prose-h1:text-5xl'>
        <BurgerMenuProvider>
          <LayoutHeader />
        </BurgerMenuProvider>
        {children}
      </body>
    </html>
  );
}
